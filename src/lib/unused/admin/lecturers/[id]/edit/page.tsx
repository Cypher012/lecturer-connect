...
import { departments, getLecturerById, type Lecturer, type Publication } from "@/lib/mock-data"

export default function EditLecturer({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const lecturerData = getLecturerById(params.id)
    if (lecturerData) {
      ...
      setFormData({
        ...
        profile_picture: lecturerData.profile_picture,
        orcid: lecturerData.orcid || "",
        scopus_id: lecturerData.scopus_id || "",
        linkedin_url: lecturerData.linkedin_url || "",
        personal_website: lecturerData.personal_website || "",
      })
      setCourses(lecturerData.courses_taught)
      setResearchAreas(lecturerData.research_areas)
      setQualifications(lecturerData.qualifications)
      setProjects(lecturerData.projects)
      setPublications(lecturerData.publications)
    }
  }, [params.id])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log("Updating lecturer:", updatedLecturer)
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-slate-900 mb-4">Lecturer Not Found</h1>
          <Button asChild>
            <Link href="/admin/lecturers">Back to Lecturers</Link>
          </Button>
        </div>
      </div>
    )
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Edit Lecturer</h1>
          <p className="text-slate-600">Update {lecturer.full_name}'s profile</p>
        </div>
      </div>
