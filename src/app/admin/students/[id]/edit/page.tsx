...
export default function EditStudent({ params }: { params: { id: string } }) {
    ...
    const loadStudent = async () => {
      try {
        ...
        if (studentData) {
          ...
          setResearchInterests(studentData.researchInterests)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await mockDatabase.updateStudentProfile(params.id, {
        ...formData,
        researchInterests,
        skills,
      })
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Edit Student</h1>
          <p className="text-slate-600">Update {student.name}'s profile</p>
        </div>
      </div>
