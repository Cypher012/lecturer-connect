...
export default function LecturersPage() {
  ...
  const allResearchAreas = useMemo(() => {
    ...
    lecturers.forEach((lecturer) => {
      lecturer.research_areas.forEach((area) => areas.add(area))
    })
  ...
  const filteredLecturers = useMemo(() => {
    ...
    // Apply search filter
    if (searchQuery.trim()) {
      filtered = searchLecturers(searchQuery)
    }
    ...
    if (selectedDepartment !== "all") {
      ...
      if (dept) {
        filtered = filtered.filter((lecturer) => lecturer.department === dept.name)
      }
    }

    // Apply research area filter
    if (selectedResearchArea !== "all") {
      filtered = filtered.filter((lecturer) => lecturer.research_areas.includes(selectedResearchArea))
    }
